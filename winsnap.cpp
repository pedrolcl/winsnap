...
Edge::Edge(int position, int start, int end)
    : Position(position)
    , Start(start)
    , End(end)
{}
...
bool WinSnap::HandleMessage(void *message)
{
    ...
    if (this->m_enabled) {
        ...
        switch (msg->message) {
        case WM_SIZING:
            return HandleSizing(*reinterpret_cast<RECT *>(msg->lParam),
...
bool WinSnap::HandleEnterSizeMove()
{
    ...
    EnumDisplayMonitors(
        nullptr,
        nullptr,
        [](HMONITOR monitorHandle, HDC, LPRECT, LPARAM param) -> BOOL {
    ...
    EnumWindows(
        [](HWND windowHandle, LPARAM _param) -> BOOL {
            ...
            if (/*(extendedStyles & WS_EX_TOOLWINDOW) != 0 ||*/ (extendedStyles & WS_EX_NOACTIVATE)
                != 0)
                return true;
    ...
    for (auto &edgeList : m_edges) {
        edgeList.sort(
            [](const Edge &_1, const Edge &_2) -> bool { return _1.Position < _2.Position; });
...
bool WinSnap::HandleMoving(RECT &bounds)
{
    ...
    RECT realBounds;
    if (m_inProgress) {
        POINT offsetDiff{cursorOffset.x - m_originalCursorOffset.x,
                         cursorOffset.y - m_originalCursorOffset.y};
        SetRect(&realBounds,
                bounds.left + offsetDiff.x,
                bounds.top + offsetDiff.y,
                bounds.right + offsetDiff.x,
                bounds.bottom + offsetDiff.y);
    } else
        realBounds = bounds;

    int boundsEdges[Side::Count]{realBounds.left,
                                 realBounds.top,
                                 realBounds.right,
                                 realBounds.bottom};
    ...
    if ((GetKeyState(VK_SHIFT) & 0x8000) == 0
        && (snapDirections[0] || snapDirections[1] || snapDirections[2] || snapDirections[3])) {
        if (!m_inProgress)
            m_inProgress = true;

        RECT snapRect = {snapEdges[0] - m_border.left,
                         snapEdges[1] - m_border.top,
                         snapEdges[2] + m_border.right,
                         snapEdges[3] + m_border.bottom};
        SnapToRect(&bounds,
                   snapRect,
                   true,
                   snapDirections[0],
...
bool WinSnap::HandleSizing(RECT &bounds, int which)
{
    ...
    allowSnap[Side::Left] = (which == WMSZ_LEFT || which == WMSZ_TOPLEFT
                             || which == WMSZ_BOTTOMLEFT);
    allowSnap[Side::Top] = (which == WMSZ_TOP || which == WMSZ_TOPLEFT || which == WMSZ_TOPRIGHT);
    allowSnap[Side::Right] = (which == WMSZ_RIGHT || which == WMSZ_TOPRIGHT
                              || which == WMSZ_BOTTOMRIGHT);
    allowSnap[Side::Bottom] = (which == WMSZ_BOTTOM || which == WMSZ_BOTTOMLEFT
                               || which == WMSZ_BOTTOMRIGHT);
...
void WinSnap::SnapToRect(RECT *bounds,
                         const RECT &rect,
                         bool retainSize,
                         bool left,
                         bool top,
...
bool WinSnap::CanSnapEdge(int boundsEdges[Side::Count], Side which, int *snapPosition) const
{
    for (const Edge &edge : m_edges[which]) {
        ...
        if (edgeDistance >= SNAP_DISTANCE)
            ...
            int min = std::min<>(boundsEdges[(which + 1) % Side::Count],
                                 boundsEdges[(which + 3) % Side::Count]);
            int max = std::max<>(boundsEdges[(which + 1) % Side::Count],
                                 boundsEdges[(which + 3) % Side::Count]);
